...
class Element{
    ...
    addProperty(key){
        let valProperty = window.getComputedStyle( this.$el ).getPropertyValue( key );
        this.properties[ key ] = valProperty;
    }

    setProperties(lsProperty){
        if(typeof lsProperty === "string"){
            this.addProperty( lsProperty );
        }
        if(typeof lsProperty === "object"){
            for(let i = 0; i < lsProperty.length; i++){
                this.addProperty( lsProperty[ i ] );
            }
        }
    }

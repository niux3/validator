 ...
 * console.log(validatePhoneNumber("+1-800-555-5555", "en-US")); // true
 *
 * // Valid French phone number
 * console.log(validatePhoneNumber("0612345678", "fr-FR")); // true
 *
 * // Invalid phone number for the specified country
 * console.log(validatePhoneNumber("12345", "en-US")); // false
 *
 * // Unsupported country
 * console.log(validatePhoneNumber("1234567890", "xx-XX")); // false
 * ```
 */
export default (value:string, country:string):boolean =>{
    let phones:{[country:string]: any} = {
    ...
    if(!Object.keys(phones).includes(country) || typeof value !== 'string'){
        return false
    }
    return phones[ country ].test(value)
}
